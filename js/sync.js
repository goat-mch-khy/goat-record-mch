...
const SYNC = (() => {
  ...
  async function fetchURL(url, options = {}) {
    ...
    try {
      ...
      if (fe.name === 'AbortError') throw new Error(`Request timed out after ${timeout/1000}s`);
      // XHR fallback
      return new Promise((res, rej) => {
        ...
        xhr.ontimeout = () => rej(new Error(`Request timed out after ${timeout/1000}s`));
        xhr.send();
      });
    }
  }
  ...
  async function fetchSheet(sheetName) {
    // Your sheet: headers at row 3, data from row 4
    // gviz headers=N means "skip N rows, use next row as headers"
    // So headers=2 skips rows 1-2, uses row 3 as headers, data from row 4
    ...
    const res = await fetchURL(url, { timeout: 60000 });
    if (!res.text) throw new Error('Empty response from Google Sheets');
  ...
  async function probe(ep) {
    ...
    try {
      const res  = await fetchURL(`${ep}?action=health&_t=${Date.now()}`, { timeout: 3000 });
