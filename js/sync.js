...
const SYNC = (() => {
  ...
  async function fetchSheet(sheetName) {
    // headers=1 means first row of returned data is headers
    // We use range=A3:ZZ to start from row 3 (where our headers are)
    const url = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(sheetName)}&range=A3:ZZ&headers=1&_t=${Date.now()}`;
